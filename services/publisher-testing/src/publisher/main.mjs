...
import { generateFacebookEvent, generateTiktokEvent, corruptFacebookEvent, corruptTiktokEvent } from "./events.mjs";

const listenerEndpoint = process.env.EVENT_ENDPOINT;
const MIN_EVENTS_PER_SECOND = parseInt(process.env.MIN_EVENTS_PER_SECOND || '10', 10);
const MAX_EVENTS_PER_SECOND = parseInt(process.env.MAX_EVENTS_PER_SECOND || '100', 10);

if (!listenerEndpoint) {
	throw new Error("EVENT_ENDPOINT is not set");
}

const eventsPerSecond = () => Math.floor(Math.random() * (MAX_EVENTS_PER_SECOND - MIN_EVENTS_PER_SECOND + 1)) + MIN_EVENTS_PER_SECOND;
...
const sendEvents = async () => {
	...
	try {
		...
		console.log(`Sent ${events.length} events successfully. Consumer response: ${responseText}`);
