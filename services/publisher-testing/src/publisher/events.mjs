...
export const generateFacebookEvent = () => {
  ...
  const eventType =
    funnelStage === "top"
      ? faker.helpers.arrayElement(["ad.view", "page.like", "comment", "video.view"])
      : faker.helpers.arrayElement(["ad.click", "form.submission", "checkout.complete"]);
      ...
      engagement:
        funnelStage === "top"
          ? {
              ...
              referrer: faker.helpers.arrayElement(["newsfeed", "marketplace", "groups"]),
              videoId: eventType === "video.view" ? `vid-${faker.string.alphanumeric(8)}` : null,
            }
          : {
              adId: `ad-${faker.string.alphanumeric(8)}`,
              campaignId: `camp-${faker.string.alphanumeric(5)}`,
              clickPosition: faker.helpers.arrayElement(["top_left", "bottom_right", "center"]),
    ...
    data: {
      ...
      engagement:
        funnelStage === "top"
          ...
          : {
              actionTime: new Date().toISOString(),
              profileId: eventType === "profile.visit" ? `profile-${faker.string.alphanumeric(6)}` : null,
              purchasedItem: eventType === "purchase" ? faker.commerce.productName() : null,
              purchaseAmount: eventType === "purchase" ? faker.commerce.price({ min: 5, max: 500 }) : null,
            },
    },
  };
};
...
export const corruptTiktokEvent = (event) => {
  ...
  if (r < 0.125) {
    ...
    if (corrupted.funnelStage === "top") {
      ...
      if (corrupted.data.engagement && corrupted.data.engagement.purchasedItem) {
